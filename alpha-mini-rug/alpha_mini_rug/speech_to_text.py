...
class SpeechToText:
    ...
    def listen_continues(self, data):
        if self.do_speech:
            ...
            if frame_single is None:
                ...
                # print(audio_np)
                try:
                    self.logger(f"noise level {audio_np[0]}")
                    self.audio_frames.append(audio_np)
                ...
                if len(self.audio_frames) > self.max_length_audio * self.sample_rate:
                    self.logger("max length reached")
    ...
    def speech_to_text(self, data):
        ...
        try:
            ...
            if self.language_setting not in languages:
                print('language not supported current options are "nl-NL" and "en-US"')
                print("using default language en-US")
                text = recognizer.recognize_google(audio_data, language=languages[1])
            else:
                text = recognizer.recognize_google(
                    audio_data, language=self.language_setting
                )
    ...
    def give_me_words(
        self,
    ):  # todo als er een andere taal is geselecteerd dan moet deze ook worden terug gegeven
